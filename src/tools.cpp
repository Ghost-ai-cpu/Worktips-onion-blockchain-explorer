...
{
    // the default folder of the lmdb blockchain database
    string default_lmdb_dir   = wtipseg::get_default_lmdb_folder(nettype);
    ...
    blockchain_path = wtipseg::remove_trailing_path_separator(blockchain_path);
...
{
    pair<uint64_t, uint64_t> sum_wtips {0, 0};
    ...
    {
        sum_wtips.first += vout["amount"].get<uint64_t>();
        ++sum_wtips.second;
    }
...
{
    pair<uint64_t, uint64_t> sum_wtips {0ULL, 0ULL};
        ...
        vector<txin_to_key>& input_key_imgs)
{
    ...
    uint64_t wtips_outputs       {0};
    uint64_t wtips_inputs        {0};
    uint64_t mixin_no          {0};
    ...
    for (size_t i = 0; i < input_no; ++i)
    {
        ...
        wtips_inputs += tx_in_to_key.amount;
    ...
    return {wtips_outputs, wtips_inputs, mixin_no, num_nonrct_inputs};
};
...
summary_of_in_out_rct(const json& _json)
{
    ...
    for (const json& vout: _json["vout"])
    {
        wtips_outputs += vout["amount"].get<uint64_t>();
    }
    ...
    return {wtips_outputs, wtips_inputs, no_outputs, no_inputs, mixin_no, num_nonrct_inputs};
};
...
sum_money_in_inputs(const string& json_str)
{
    ...
    for (json& vin: j["vin"])
    {
        sum_wtips.first += vin["key"]["amount"].get<uint64_t>();
        ++sum_wtips.second;
    }
...
{
    array<uint64_t, 2> sum_wtips;

    sum_wtips[0] = sum_money_in_inputs(tx);
    sum_wtips[1] = sum_money_in_outputs(tx);
...
{
    array<uint64_t, 2> sum_wtips {0,0};
